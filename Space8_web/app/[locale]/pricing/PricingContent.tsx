...
import { Sunrise, Sun, Moon, Circle, Triangle, Target } from "lucide-react";
...
// ── Part 2: Sectioned Pricing — WHITE background, one period per screen. ──
// Extracted as its own export so the homepage can reuse the EXACT SAME
// pricing block as /pricing (item 四 of the spec) without also pulling in
// the standalone page's hero/FAQ/CTA sections.
export function SectionedPricing({ periods }: { periods: PricingPeriod[] }) {
  const t = useTranslations("pricingPage");

  return (
    <>
      {periods.map((period) => {
        const hasLongBookingDiscount =
          period.discountRate !== undefined && period.discountMinHours !== undefined;
        const Icon = period.id === "morning" ? Sunrise : period.id === "afternoon" ? Sun : Moon;
              ...
              >
                ...
                >
                  ...
                  {hasLongBookingDiscount && (
                    ...
                    >
                      <Mark color={GREEN}>
                        <CMSText k="pricingPage.long_booking_discount_prefix">
                          {t("long_booking_discount_prefix", { hours: period.discountMinHours as number })}
                        </CMSText>{" "}
                        {fmt(period.discountRate as number)}
                      </Mark>
                    </div>
                  )}
                </div>
...
export default function PricingContent({
  ...
  return (
    <div style={{ fontFamily: FONT_FAMILY }}>
      ...
      {/* ── Part 2: Sectioned Pricing ── */}
      <SectionedPricing periods={periods} />
