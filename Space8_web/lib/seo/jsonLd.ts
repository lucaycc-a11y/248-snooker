import { getVenueAddress } from "@/lib/venue";
...
//
// NOTE: priceRange below is a display string for search engines, not a
// charged amount — the real per-period rates still live only in the
// `config` table (see lib/pricing.ts). Update this string manually if
// those rates change; it is not read from config at build/request time.
export function buildSportsClubJsonLd(locale: string, path: string) {
  return {
    "@context": "https://schema.org",
    "@type": "SportsClub",
    name: "Space8",
    description: "香港首間自助中式桌球預訂平台，每日 06:00 至 24:00 營業",
    url: `${BASE}${locale === "zh-HK" ? path : `/${locale}${path}`}`,
    telephone: "+85264274620",
    email: "info.formhk@gmail.com",
    address: {
      "@type": "PostalAddress",
      streetAddress: getVenueAddress(locale),
      addressCountry: "HK",
