...
const PERIOD_IDS: PricingPeriod['id'][] = ['afternoon', 'evening', 'latenight']
...
function validatePeriod(value: unknown): PricingPeriod | null {
  if (!isRecord(value)) return null
  const { id, rate, start, end, days } = value
  ...
  return { id: id as PricingPeriod['id'], rate, start, end, days: days as PricingPeriod['days'] }
}
