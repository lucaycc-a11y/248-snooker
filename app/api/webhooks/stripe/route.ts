...
async function handleSucceeded(
  ...
  try {
    // Send confirmation email via the new template-based sender
    // which fetches all data directly from the DB (no client-submitted values).
...
async function handleGroupSucceeded(
  ...
  try {
    ...
    if (userId && primaryId) {
      // Send confirmation email via the new template-based sender
      const { sendBookingConfirmation } = await import('@/lib/resend/template-send')
      await sendBookingConfirmation(primaryId)
    }
    await supabase.from('notification_log').insert(
