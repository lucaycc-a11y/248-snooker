...
function SettingsTab({ user, bookings, onSignOut }: { user: MemberData["user"]; bookings: MemberBooking[]; onSignOut: () => void }) {
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => setPwCooldown((c) => c - 1), 1000);
    return () => window.clearTimeout(id);
  }, [pwCooldown]);
  ...
  // ── Save handler ──────────────────────────────────────────────────────────
  // Saves display name only. Notification toggles persist on change via
  // toggleNotif. Phone and password are deliberately NOT saved here — both
  // require the emailed single-use link (requirement B).
  const save = async () => {
    setSaving(true);
    setSaveError(null);
    try {
  ...
  const requestChangeLink = async (purpose: "phone" | "password") => {
    ...
    setMessage(null);
    try {
      const res = await fetch("/api/auth/change-request", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!res.ok) {
        if (res.status === 429) {
          setMessage(t("settings_change_rate_limited"));
        } else if (j?.error === "no_email_on_account") {
          setMessage(t("settings_change_no_email"));
        } else {
          setMessage(t("settings_err_generic"));
        }
        return;
      }

      setMessage(t("settings_change_link_sent"));
      setCooldown(CHANGE_REQUEST_COOLDOWN);
    } catch {
      setMessage(t("settings_err_generic"));
    } finally {
      setSending(false);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      ...
      {/* Change password — same endpoint as 更改電話, different purpose */}
      <button
        type="button"
        onClick={() => requestChangeLink("password")}
        disabled={pwSending || pwCooldown > 0}
        style={{
          minHeight: 48,
          borderRadius: "12px",
          border: `1px solid ${BORDER}`,
          background: "transparent",
          color: pwCooldown > 0 ? SUBTLE : INK,
          fontSize: "15px",
          fontWeight: 600,
          cursor: pwSending || pwCooldown > 0 ? "default" : "pointer",
      ...
      >
        {pwCooldown > 0
          ? t("settings_change_resend_in", { seconds: pwCooldown })
          : pwSending
            ? t("sending")
