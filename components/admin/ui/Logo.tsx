...
export default function Logo({ href = '/admin', className = '', compact = false }: LogoProps) {
  return (
    <Link
      href={href}
      aria-label="SPACE8"
      className={`admin-ui-logo ${compact ? 'admin-ui-logo--compact' : ''} ${className}`}
    >
      <svg viewBox={compact ? '0 0 42 42' : '0 0 196 42'} aria-hidden="true">
        <path
          fill="currentColor"
