...
// without ever checking profile_complete. A client that skipped or dismissed the
// profile step still holds a valid session, so it could drive a booking straight
// through the API and end up with a paid slot owned by an account carrying no
// verified name, email or phone — the exact state the profile step exists to
// prevent. A UI-only gate is not a gate.
//
...
export async function requireCompleteProfile(
  ...
  const { data, error } = await supabase
    ...
    .eq('id', userId)
    .maybeSingle<{ profile_complete: boolean | null }>()
