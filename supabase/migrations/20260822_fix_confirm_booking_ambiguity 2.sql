-- Drop ALL existing confirm_booking overloads, then recreate the correct one.
--
-- WHY: The DB has accumulated multiple overloads of confirm_booking across
-- migrations 0004, 0008, and possibly manual SQL-editor changes, causing an
-- "ambiguous function call" error when the code calls confirm_booking with
-- 5 named parameters. The correct version is the 0008 secure signature:
--   confirm_booking(uuid, text, text, text, text)
-- which reads price/free status from the bookings row (not from the caller).
-- All 3 call sites (admin-test-confirm, kpay webhook, stripe webhook) use
-- this signature.
--
-- This migration:
--   1. Drops EVERY overload of confirm_booking (including the 0004 insecure
--      version and any manually-created variants in the live DB).
--   2. Recreates the 0008 secure version with the correct signature.
--   3. Locks EXECUTE down to service_role only (P0 security fix).
--   4. Also drops the companion confirm_booking_group if it exists with
--      the wrong signature (safety measure).
--
-- Run this in the Supabase SQL Editor for project wqmciwieiqvnswvspdyz.
-- Idempotent: safe to run multiple times.

-- ═══════════════════════════════════════════════════════════════════════════
-- Step 1: Nuke every overload of confirm_booking
-- ═══════════════════════════════════════════════════════════════════════════
do $$
declare
  rec record;
...
-- ═══════════════════════════════════════════════════════════════════════════
-- Step 2: Recreate the correct 0008 version
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
-- Step 3: Grants — only service_role may call this
-- ═══════════════════════════════════════════════════════════════════════════
...
begin
  ...
  if v_count = 0 then
    raise exception 'FATAL: confirm_booking was dropped but not recreated';
  elsif v_count > 1 then
    raise warning 'Multiple overloads still exist (count=%) — ambiguity may persist', v_count;
  else
    raise notice 'confirm_booking: exactly 1 overload — unambiguous ✓';
  end if;
end $$;